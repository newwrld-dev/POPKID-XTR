...
// Improved Helper: Works with both Boolean (from your config) and String (from .env)
const isEnabled = (val) => {
        ...
        Matrix.ev.on('messages.upsert', async (chatUpdate) => {
            try {
                ...
                if (remoteJid === 'status@broadcast') {
                    ...
                    // 2. Auto Status Reaction (Using your config key: AUTO_STATUS_REACT)
                    ...
                    // 3. Auto Status Reply (Using your config key: AUTO_REPLY_STATUS)
                    if (isEnabled(config.AUTO_REPLY_STATUS)) {
                        const replyMsg = config.STATUS_READ_MSG || 'Status Viewed by popkid';
                ...
                console.error('Error in Master Listener:', err);
            }
        });
