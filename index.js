...
conn?.ev?.on('messages.update', async updates => {
  ...
  // FIXED STATUS REACT SECTION
  if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_REACT === "true"){
    const ravlike = await conn.decodeJid(conn.user.id);
    
    // Checks config for custom emojis, otherwise uses default list
    let emojis;
    if (config.CUSTOM_STATUS_EMOJIS) {
        emojis = config.CUSTOM_STATUS_EMOJIS.split(',');
    } else {
        emojis = ['🟠', '🟡'];
    }
