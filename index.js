...
const clearTempDir = () => {
  fs.readdir(tempDir, (err, files) => {
    if (err) return console.error(chalk.red("[❌] Error clearing temp dir:", err.message));
...
async function connectWithPairing(conn, useMobile) {
  ...
  if (!process.stdin.isTTY) {
    console.error(chalk.red("❌ Cannot prompt for number in non-interactive environment"));
    process.exit(1);
  }
...
async function connectToWA() {
  ...
  const conn = makeWASocket({
    ...
    printQRInTerminal: !fs.existsSync(credsPath) && !pairingCode,
  ...
  if (pairingCode && !state.creds.registered) await connectWithPairing(conn, useMobile);

  conn.ev.on('connection.update', async (update) => {
    const { connection, lastDisconnect, qr } = update;

    // ✅ Fix: Handle QR safely
    if (qr) {
      console.log(chalk.yellow("📱 Scan this QR to link WhatsApp:"));
    ...
    if (connection === 'close') {
      ...
      if (reason === DisconnectReason.loggedOut) {
        console.log(chalk.red('[ 🛑 ] Logged out, please update SESSION_ID'));
      ...
      console.log(chalk.green('[ 🤖 ] Connected ✅'));
            ...
            try {
                ...
                const username = "kenyanpopkid";
                const repoUrl = "https://github.com/kenyanpopkid/POPKID-XTR";
                
                const upMessage = `*╭──〔 𝗰𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱 〕───⊷
│ Prefix: ${prefix}
...
> Report any error to the dev`;
        ...
        "120363289379419860@newsletter",
            ...
            const inviteCode = "BRh9Hn12AGh7AKT4HTqXK5"; // Replace with your group invite code
            if (inviteCode) {
                try {
  ...
  try {
    ...
    for (const call of calls) {
      ...
      await conn.sendMessage(from, {
        text: config.REJECT_MSG || '*📞 𝗰𝗮𝗹𝗹𝘀 𝗻𝗼𝘁 𝗮𝗹𝗹𝗼𝘄𝗲𝗱 𝗯𝘆 𝗽𝗼𝗽𝗸𝗶𝗱 𝘅𝘁𝗿 📵*'
      });
...
BotActivityFilter(conn);	
 ...
 /// READ STATUS       
  ...
  const udp = botNumber.split('@')[0];
    const immumd = ('254732297194', '254111385747');
