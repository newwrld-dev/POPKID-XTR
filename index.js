      ...
      if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
          ...
          filer.download((err, data) => {
              ...
              fs.writeFile(__dirname + '/sessions/creds.json', data, (writeErr) => {
                  if (writeErr) {
                      console.error("❌ Failed to save session file:", writeErr);
                      return;
                  }
                  console.log("✅ SESSIO-ID CONNECTED and saved successfully.");
                  callback(); // Proceed to connection after successful save
              });
          });
      } else {
          console.log("Session file already exists. Starting connection.");
          callback(); // Proceed directly if file exists
      }
  }
...
// --- NEW: Newsletter Follow ---
  try {
    conn.newsletterFollow("120363289379419860@newsletter");
...
if (!isReact && senderNumber === botNumber) {
  ...
  // Start the initialization process, then connect to WA
  initializeSession(() => {
    // A slight delay before connecting to allow file system operations to settle
    setTimeout(() => {
        connectToWA();
    }, 4000);
  });
